...
- [ ] T011 [P] (If frontend in scope) Initialize Angular 22 LTS app consuming existing backend API
- [ ] T012 [P] (If frontend in scope) Implement frontend auth integration using current backend login mechanism
- [ ] T013 [P] (If frontend in scope) Add role-aware navigation/action visibility (`ADMIN` write actions, `USER` read-only UI)
- [ ] T014 [P] (If frontend in scope) Configure Cypress E2E baseline and critical flow suites
- [ ] T015 (If frontend in scope) Explicitly justify frontend Docker only if required; otherwise keep it out of MVP scope
...
- [ ] T021 [US1] Implement [endpoint/feature] in src/[location]/[file].py under `/api/v1`
